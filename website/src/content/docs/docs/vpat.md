...
exists today so a reader can see exactly how much of each row's claim is backed by a real check versus
still open. See the [accessibility statement](/docs/accessibility-statement/) for the same status in
prose.
...
|---|---|---|
| 1.1.1 Non-text Content | Not Evaluated | Automated axe-core WCAG 2.2 AA scan (every page, both themes, in CI) reports no violations for this rule; not yet manually verified. |
| 1.3.1 Info and Relationships | Not Evaluated | Automated axe-core scan reports no violations; not yet manually verified. |
...
| 1.4.1 Use of Color | Not Evaluated | Automated axe-core scan reports no violations; not yet manually verified. |
| 1.4.3 Contrast (Minimum) | Not Evaluated | Automated axe-core `color-contrast` rule reports no violations across every page and theme; not yet manually verified. |
...
| 1.4.11 Non-text Contrast | Not Evaluated | Automated axe-core scan reports no violations; not yet manually verified. |
...
| 2.5.8 Target Size (Minimum) | Not Evaluated | Automated axe-core scan reports no violations for `target-size`; not yet manually verified. |
| 3.1.1 Language of Page | Not Evaluated | Automated axe-core `html-has-lang` rule reports no violations; not yet manually verified. |
...
| 4.1.2 Name, Role, Value | Not Evaluated | Automated axe-core scan (`button-name`, `image-alt`, and related rules) reports no violations; not yet manually verified. |
...
| 11.5.2.5 Objects that are Programmatically Determinable | Not Evaluated | Automated axe-core scan reports no violations; not yet manually verified. |
