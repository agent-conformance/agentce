...
Run as ``python -m agentce_adapters.check_support_matrix <adapter-dir>`` (the item-2.2 acceptance uses
...
* the event types the matrix declares are exactly those the fixtures emit;
...
def _observed(adapter_root: Path) -> tuple[dict[str, set[str]], set[str], list[str]]:
    ...
    for fixture in discover_fixtures(adapter_root / "fixtures"):
        ...
        if (
            ...
            and result.source_manifest != fixture.expected_manifest
        ):
            problems.append(
                f"fixture {fixture.name}: source manifest does not match expected-manifest.json"
            )
