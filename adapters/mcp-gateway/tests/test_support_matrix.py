...
def _allowed_paths(event_type: str) -> set[str]:
    ...
    for name, spec in payload.items():
        ...
        # A member may be named as a whole object or by a dotted leaf ("principal.role").
        paths.add(name)
        if sub_props:
...
def test_check_detects_an_under_claimed_member(tmp_path: Path) -> None:
    def drop_member(matrix: dict[str, Any]) -> None:
        matrix["events"]["ToolCall"]["members"].remove("refs.authorization")

    root = _broken_copy(tmp_path, drop_member)
    assert any("ToolCall.members" in failure for failure in check(root))
    assert main([str(root)]) == 1


def test_check_detects_a_populated_member_claimed_missing(tmp_path: Path) -> None:
    def mislabel(matrix: dict[str, Any]) -> None:
        matrix["events"]["ToolCall"]["missing"].append("tool.name")
...
def test_check_detects_an_unexercised_convention(tmp_path: Path) -> None:
    def add_convention(matrix: dict[str, Any]) -> None:
        matrix["conventions"].append("mcp:9999-99-99")
...
def test_check_detects_a_wrong_adapter_name(tmp_path: Path) -> None:
    def rename(matrix: dict[str, Any]) -> None:
        matrix["adapter"] = "not-mcp-gateway"
...
def test_check_detects_drift_between_output_and_expected(tmp_path: Path) -> None:
    ...
    (root / "fixtures" / "tools-call-enforced" / "expected.jsonl").write_text(
        "{}\n", encoding="utf-8"
    )
    assert any("does not match expected.jsonl" in failure for failure in check(root))


def test_check_detects_source_manifest_drift(tmp_path: Path) -> None:
    root = tmp_path / "adapter"
    root.mkdir()
    shutil.copytree(ROOT / "fixtures", root / "fixtures")
    shutil.copy(ROOT / "support-matrix.yaml", root / "support-matrix.yaml")
    (root / "fixtures" / "tools-call-enforced" / "expected-manifest.json").write_text(
        '{"counts": {}}\n', encoding="utf-8"
    )
    assert any("source manifest does not match" in failure for failure in check(root))
...
def test_members_of_dots_one_level_and_skips_framing() -> None:
    event = {
        "data": {
            ...
            "token_ref": "t",
            "chain": [{"id": "a", "kind": "service"}],
            "verification": {"status": "verified", "method": "m"},
            "integrity": {"hash": "h"},
        }
    }
