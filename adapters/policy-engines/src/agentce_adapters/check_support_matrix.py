...
Run as ``python -m agentce_adapters.check_support_matrix <adapter-dir>`` (the item-2.3 acceptance uses
``.``). The check runs the adapter over every fixture and holds ``support-matrix.yaml`` to the
evidence the fixtures actually demonstrate: the ``adapter`` name matches; the declared event types are
exactly those the fixtures emit; each type's declared ``members`` equal the members the adapter
populates across the fixtures; nothing under ``missing`` is ever populated; and the declared
``conventions`` are exactly those the fixtures exercise (one per engine). Prints ``SUPPORT MATRIX OK``
on success. Standard library plus PyYAML; no network.
"""
...
def _observed(adapter_root: Path) -> tuple[dict[str, set[str]], set[str], list[str]]:
    """Return (members per event type, conventions, drift problems) observed across fixtures."""
