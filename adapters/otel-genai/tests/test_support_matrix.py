...
def _object_ref(spec: dict[str, Any]) -> str | None:
    """Return the definition name if ``spec`` is an (inlined) object property, else ``None``."""
    if "$ref" in spec:
...
def _allowed_paths(event_type: str) -> set[str]:
    """Every member path the model defines for ``<event_type>Payload`` (SPEC 12.3 granularity)."""
    ...
    for name, spec in payload.items():
        ...
        # A member may be named as a whole object ("deployer") or by a dotted leaf ("model.provider").
        paths.add(name)
        if sub_props:
...
def test_every_declared_path_is_a_real_model_member() -> None:
    # Guards against typos in members/missing that the consistency check alone would not catch.
...
def test_members_and_missing_are_disjoint_by_construction() -> None:
    for event_type, spec in MATRIX["events"].items():
        ...
        assert members.isdisjoint(missing), event_type
...
def test_check_detects_drift_between_output_and_expected(tmp_path: Path) -> None:
    ...
    # Corrupt one fixture's recorded expectation so the adapter output no longer matches it.
    (root / "fixtures" / "otel-genai-chat" / "expected.jsonl").write_text(
        "{}\n", encoding="utf-8"
    )
