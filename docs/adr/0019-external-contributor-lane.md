...
1. **Two lanes, one rule each, always evaluated together.** A commit authored by the maintainer's own
   identity keeps the exact-identity rule: every `Signed-off-by` on it must be that same identity too.
   A commit authored by anyone else is DCO-standard: accepted only when at least one `Signed-off-by`
   ...
   bypass it.
2. **A real, offline, required check for the external lane.** `tools/dco_check.py` implements the
...
3. **`CONTRIBUTING.md` documents the path**: fork, sign off as yourself, open a pull request against
   ...
   not opened.
4. **Automated dependency-update pull requests (for example, Dependabot) follow the external lane**,
   not a special case: their commits are authored and signed off by the bot itself, under the same
   DCO rule as any other outside contributor. No separate mechanism is created for them.
5. **Making the `dco` check a required status check on the protected `main` branch, and deciding how a
...
- **No weakening.** The exact-identity rule for the maintainer's and the project's own automated
  commits is unchanged; the message denylist still applies to every commit on every lane. Only the
